...
signal dataram: ram:=(("00110000"), ("00110000"), ("00111010"), ("00110000"),
	...
	x"80", x"80", x"5A", x"51", x"2D" x"31", x"34", x"30", x"33", x"31", x"32", x"30", x"35", x"80", x"80", x"80");	
//若为www.BUAA.edu.cn（x"77", x"77", x"77", x"2E", x"42", x"55", x"41", x"41", x"2E", x"65", x"64", x"75", x"2E", x"63", x"6e", x"80"）
...
If (c1=N/2-1) then//////250K时钟分频
	c1:=0;
	clk0:=not clk0;
else
	c1:=c1+1;
end if;
	if c2=50000000/2-1 then//1Hz时钟分频，用于时钟计数
		c2:=0;
		clk1:=not clk1;
	else
		c2:=c2+1;
...
write:
	...
	begin
	if (clk_250Khz'event and clk_250Khz='1') then//将时钟计数结果存于dataram中，前面加“0011”是
		dataram(0)<="0011" & hour_h_tmp;	//根据0-9对应地址和数字的关系得到的
	dataram(1)<="0011" & hour_l_tmp;	//此处的时钟用ms级就可以，因为时钟每隔1秒才刷新一次
...
control:
	...
	when clear_lcd=>//清屏，文档中要求至少延时1.64ms
	oe<='1';
	rs<='0';
	rw<='0';
	data<=x"01";
	cnt1<=cnt1+1;
if cnt1>delay*1 and cnt1<=delay*6 then//延时操作
	oe<='0';	//保证液晶有足够的使能时间
else
	oe<='1';
end if;
...
when entry_set=>//写入新数据后光标右移，写入新数据后显示屏不移动
		oe<='1';
		rs<='0';
		rw<='0';
		data<=x"06";
		cnt1<=cnt1+1;
if (cnt1>delay*1 and cnt1<=delay*2 )then//延时操作
	oe<='0';	//保证液晶有足够的使能时间
else
	oe<='1';
end if;
if (cnt1=delay*3 )then
	...
	when display_set=>//显示方式：控制显示器开/关、光标显示/关闭以及光标是否闪烁
	oe<='1';
	rs<='0';
	rw<='0';
	data<=x"0C";
	cnt1<=cnt1+1;
if (cnt1>delay and cnt1<=delay*2 )then//延时操作
	oe<='0';	//保证液晶有足够的使能时间
else
	oe<='1';
end if;
...
when funtion_set=>//功能设置：设定数据总线位数、显示的行数及字型
	oe<='1';//设置为8位并行，显示2行，5*7点阵显示
	rs<='0';
	rw<='0';
	data<=x"38";
	cnt1<=cnt1+1;
...
when position_set1=>		//设置显示数码的初始位置
	oe<='1';
	rs<='0';
	rw<='0';
	data<=x"38";
...
when position_set2=>//设置显示数据的初始位置，第二行的地址设置
	oe<='1';
	rs<='0';
	rw<='0';
	data<=x"c0";
